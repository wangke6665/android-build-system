...
public class WiFiDialog extends Dialog {
    ...
    public void show() {
        ...
        this.setCanceledOnTouchOutside(false);//对话框不可退出
        super.show();
    }
    ...
    public static class Builder {
        ...
        public WiFiDialog creates() {
            LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            // instantiate the dialog with the custom Theme
            final WiFiDialog dialog = new WiFiDialog(context, R.style.WiFiDialog);
            View v = inflater.inflate(R.layout.forget_dialog, null);
            dialog.addContentView(v, new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
            // set the confirm button
            if (positiveButtonText != null) {
                ((Button) v.findViewById(R.id.forget_dialog_bt1)).setText(positiveButtonText);
                if (positiveButtonClickListener != null) {
                    ((Button) v.findViewById(R.id.forget_dialog_bt1)) .setOnClickListener(new View.OnClickListener() {
                        public void onClick(View v) {
                            positiveButtonClickListener.onClick(dialog,DialogInterface.BUTTON_POSITIVE);
                        }
                    });
                ...
                // if no confirm button just set the visibility to GONE
                v.findViewById(R.id.forget_dialog_bt1).setVisibility( View.GONE);
            }
            // set the forget button
            if (neutralButtonText != null) {
                ((Button) v.findViewById(R.id.forget_dialog_bt2)).setText(neutralButtonText);
                if (neutralButtonClickListener != null) {
                    ((Button) v.findViewById(R.id.forget_dialog_bt2)).setOnClickListener(new View.OnClickListener() {
                        public void onClick(View v) {
                            neutralButtonClickListener.onClick(dialog,DialogInterface.BUTTON_NEUTRAL);
                        }
                    });
                ...
                // if no forget button just set the visibility to GONE
                v.findViewById(R.id.forget_dialog_bt2).setVisibility(View.GONE);
            }
            if (contentView != null) {
                // if no message set
                // add the contentView to the dialog body
                ((LinearLayout) v.findViewById(R.id.forget_dialog_content)).removeAllViews();
                ((LinearLayout) v.findViewById(R.id.forget_dialog_content)).addView(contentView,new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT));
            }
            dialog.setContentView(v);
            return dialog;
        }
