...
//                        mInternalPlayer.enableHardwareDecoder(GSYVideoType.isMediaCodec());
                    ...
                    //埋点日志上报功能默认开启，当traceId设置为sDisableAnalytic时，则关闭埋点日志上报。当traceId设置为其他参数时，则开启埋点日志上报。
...
//                        System.out.println("解码模式：硬解1" + mHeaders);
                        PlayerConfig config = mInternalPlayer.getConfig();
                        config.mUserAgent = IjkVideoView.userAgent;
                        config.mReferrer = IjkVideoView.Referer;
                        mInternalPlayer.setConfig(config);
...
//                        mInternalPlayer.getConfig().setCustomHeaders(itemsArray);
                    ...
                    mInternalPlayer.enableHardwareDecoder(IjkVideoView.DECODE);
//                    System.out.println("解码模式：硬解" + IjkVideoView.DECODE );
...
//                    config.mNetworkTimeout = 5000;
                    config.mNetworkTimeout = SharePreferenceDataUtil.getSharedIntData(mAppContext, Constant.mJ, 15);
                    //设置超时重试次数。每次重试间隔为networkTimeout。networkRetryCount=0则表示不重试，重试策略app决定，默认值为2
                    config.mNetworkRetryCount = SharePreferenceDataUtil.getSharedIntData(mAppContext, Constant.Me, 5);
...
//                    config.mReferrer = IjkVideoView.Referer;
//                    --------
//                    config.mUserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.5359.125 Safari/537.36";
//                    config.mReferrer = "http://www.soowle.cn/";
//                    --------
//                    String[] headers = new String[1];
//                    headers[0]="Host: example.com";//比如需要设置Host到header中。
//                    --------
//                    String[] headers = new String[2];
//                    headers[0]="User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.5359.125 Safari/537.36";
//                    headers[1]="Referrer: http://www.soowle.cn/";
//                    --------
...
//            System.out.println("解码模式：硬解" + percent );
